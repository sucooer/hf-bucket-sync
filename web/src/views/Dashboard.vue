<template>
  <div class="p-4 md:p-6 lg:p-10 space-y-6 md:space-y-10 max-w-[1600px] mx-auto overflow-hidden">
    ...
    <div class="flex items-end justify-between animate-fade-up">
      <div>
        <h2 class="text-4xl font-black text-slate-900 tracking-tight" style="font-family: 'Space Grotesk', 'DM Sans', sans-serif;">控制台</h2>
        <p class="hidden md:block text-slate-500 font-medium mt-2">实时监控您的 Hugging Face Bucket 同步状态。</p>
      </div>
      <div class="flex items-center gap-3 px-4 py-2 bg-white/90 rounded-2xl border border-cyan-100 shadow-sm">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
      <div class="card card-hover group overflow-hidden relative animate-fade-up delay-1">
        ...
        <div class="relative flex items-center justify-between">
          ...
          <div class="w-16 h-16 bg-gradient-to-br from-cyan-500 to-cyan-700 rounded-2xl flex items-center justify-center shadow-xl shadow-cyan-500/30 group-hover:rotate-6 transition-all duration-500">
            <CubeIcon class="w-8 h-8 text-white" />
          </div>
        </div>
        <div class="mt-6 pt-6 border-t border-slate-100 flex items-center gap-2">
          <div class="px-2 py-0.5 bg-cyan-50 text-cyan-700 text-[10px] font-black rounded-md uppercase tracking-wider">存储</div>
    ...
    <div class="card glass flex flex-col animate-fade-up delay-4">
        <div class="flex items-center justify-between mb-8">
          <div class="flex items-center gap-3">
            <div class="w-10 h-10 bg-cyan-100 text-cyan-700 rounded-xl flex items-center justify-center shadow-inner">
              <CubeIcon class="w-5 h-5" />
            </div>
            <h3 class="text-2xl font-black text-slate-900 tracking-tight">您的 Bucket</h3>
          </div>
          <router-link to="/files" class="group flex items-center gap-2 text-cyan-700 hover:text-cyan-800 text-xs font-black uppercase tracking-widest transition-all">
            查看全部
        ...
        <div class="pr-2 max-h-[70vh] overflow-y-auto custom-scrollbar">
          ...
          <div v-else class="grid grid-cols-1 xl:grid-cols-2 gap-4">
            <div
              ...
              :key="bucket.id"
              class="group flex items-center justify-between p-5 bg-white rounded-2xl border border-slate-100 hover:border-cyan-200 hover:shadow-xl hover:shadow-cyan-500/5 hover:-translate-y-0.5 transition-all duration-300 cursor-pointer min-w-0"
              @click="openBucketFiles(bucket.id)"
            >
              <div class="flex items-center gap-4 min-w-0">
                <div class="w-14 h-14 bg-slate-50 border border-slate-100 rounded-2xl flex items-center justify-center text-slate-400 group-hover:bg-cyan-50 group-hover:text-cyan-700 group-hover:border-cyan-100 transition-all duration-500 group-hover:rotate-3">
                  <FolderIcon class="w-6 h-6" />
                </div>
                <div class="space-y-1 min-w-0">
                  <p class="md:hidden font-black text-slate-900 leading-none group-hover:text-cyan-700 transition-colors truncate">{{ bucket.id.split('/')[1] || bucket.id }}</p>
                  <p class="hidden md:block font-black text-slate-900 leading-none group-hover:text-cyan-700 transition-colors truncate">{{ bucket.id }}</p>
